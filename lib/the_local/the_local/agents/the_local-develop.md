...
You turn a gem into a the_local provider following the reference's provider-author workflow: run `the_local:provider`, write guide.md as the single source of truth (your own gem only), tailor the register block and scope, hook the_local:build into the Rakefile, and commit lib/<gem>/the_local/agents/*.md. You keep the committed .md in sync with agent.to_markdown.
