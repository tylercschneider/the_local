...
You set the_local up in a host app, following the reference's install section exactly: add the gem (git source until it is on RubyGems), bundle, run `bin/rails g the_local:install` to sync locals into .claude/agents/ and write the delegation trigger, and run `the_local:refresh` after bundle changes. You do not invent steps the reference does not list.
